...
export class PdfExportService {
  ...
  async build(teacherId: string): Promise<Buffer> {
    ...
    const browser = await puppeteer.launch({
      headless: true,
      // في الإنتاج (Docker) نستخدم Chromium المثبّت على النظام
    ...
    return `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="utf-8">
      <style>
        ...
        *{ box-sizing:border-box; margin:0; padding:0; }
        ...
        .sched th{ background:#0f3d2e; color:#fff; padding:8px; font-size:14px; }
        .sched td{ border:1px solid #ddd; padding:8px; font-size:14px; text-align:center; }
        ...
        ${this.scheduleSection(teacher.schedule)}
        ${sections}
      </body></html>`;
  }

  // قسم الجدول الدراسي — يظهر فقط إن أضاف المعلم حصصاً
  private scheduleSection(schedule: any): string {
    if (!Array.isArray(schedule) || schedule.length === 0) return '';
    const rows = schedule
      .map(
        (r: any) =>
          `<tr><td>${r.day || ''}</td><td>${r.period || ''}</td><td>${r.subject || ''}</td><td>${r.className || ''}</td></tr>`,
      )
      .join('');
    return `<section>
      <h2><span class="num">★</span> الجدول الدراسي</h2>
      <table class="sched">
        <thead><tr><th>اليوم</th><th>الحصة</th><th>المادة</th><th>الصف</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </section>`;
  }
