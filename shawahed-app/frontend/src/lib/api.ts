...
export const api = {
  ...
  changeAdminCode: (newCode: string) =>
    req('/admin/code', { method: 'PATCH', body: JSON.stringify({ newCode }) }),
  // الإعلانات/التنبيهات
  announcements: () => req('/announcements'),
  dismissAnnouncement: (id: string) =>
    req(`/announcements/${id}/dismiss`, { method: 'POST' }),
  sendAnnouncement: (message: string) =>
    req('/announcements', { method: 'POST', body: JSON.stringify({ message }) }),
  allAnnouncements: () => req('/announcements/all'),
