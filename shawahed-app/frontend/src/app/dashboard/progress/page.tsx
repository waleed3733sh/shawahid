      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-8 text-center">
        <div className="relative w-44 h-44 mx-auto mb-6">
          ...
          <div className="absolute inset-0 grid place-items-center">
            <div>
              <div className="font-display text-4xl font-extrabold text-navy-800">
                {p.pct}%
              </div>
        ...
        <p className="text-slate-600">
          أكملت <b className="text-navy-700">{p.done}</b> من أصل{' '}
          <b>{p.total}</b> بنود
        </p>
      </div>
          ...
          {indicators.map((i) => {
            ...
            return (
              <div key={i.id} className="flex items-center gap-3 text-sm">
                ...
                <span className={`flex-1 ${c > 0 ? '' : 'text-slate-400'}`}>
                  {i.titleAr}
                </span>
                <span className="text-slate-400">{c}/4</span>
              </div>
            );
          })}
        </div>
