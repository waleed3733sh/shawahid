...
export default function HomePage() {
  ...
  return (
    <div className="space-y-6">
      {/* بطاقة الترحيب */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-royal-900 via-royal-800 to-royal-700 p-8 text-white shadow-card">
        <div className="absolute top-[-30%] left-[-10%] w-72 h-72 rounded-full bg-gold-500/10 blur-3xl" />
        <div className="absolute bottom-[-40%] right-[-5%] w-80 h-80 rounded-full bg-royal-500/20 blur-3xl" />
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          [p.pct + '%', 'نسبة الإنجاز', 'from-royal-600 to-royal-800'],
          [`${p.done}/${p.total}`, 'بنود مكتملة', 'from-royal-600 to-royal-800'],
          [totalImages, 'إجمالي الشواهد', 'from-gold-500 to-gold-600'],
        ].map(([v, l, grad]) => (
          <div key={l as string} className="bg-white rounded-2xl p-5 shadow-card border border-gray-50">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${grad} mb-3`} />
            <div className="font-display text-2xl font-extrabold text-royal-900">{v}</div>
      ...
      {/* روابط سريعة */}
      <div>
        <h2 className="font-display font-extrabold text-royal-900 mb-3">إجراءات سريعة</h2>
        <div className="grid sm:grid-cols-3 gap-4">
          {quick.map(([title, desc, href, icon]) => (
            <button
              key={href}
              onClick={() => router.push(href)}
              className="group text-right bg-white rounded-2xl p-5 shadow-card border border-gray-50 hover:border-royal-400 hover:shadow-glow transition-all"
            >
              <div className="text-2xl mb-3">{icon}</div>
              <div className="font-bold text-royal-900 group-hover:text-royal-700">{title}</div>
              <div className="text-sm text-gray-400 mt-1">{desc}</div>
              <div className="text-royal-500 text-sm font-bold mt-3 opacity-0 group-hover:opacity-100 transition">
                انتقل ←
              </div>
            </button>
          ))}
