...
export default function HomePage() {
  ...
  // إغلاق تنبيه — يختفي فوراً ولا يظهر ثانيةً
  ...
  return (
    <div className="space-y-6">
      {/* تنبيهات الإدارة — تظهر مرة واحدة وتُغلق بالضغط على X */}
      {announcements.map((a) => (
        <div
          key={a.id}
          className="relative bg-gold-400/15 border border-gold-400/40 rounded-2xl p-4 pl-12"
        >
          <div className="flex items-start gap-3">
            <span className="text-xl">📢</span>
            <p className="text-royal-900 leading-relaxed flex-1 whitespace-pre-wrap">
              {a.message}
            </p>
          </div>
          <button
            ...
            aria-label="إغلاق"
            className="absolute top-3 left-3 w-7 h-7 rounded-full bg-white/70 hover:bg-white text-royal-700 grid place-items-center text-sm transition"
          >
            ✕
          </button>
        </div>
